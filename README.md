...
Lobby participants, including spectators, can use **Lobi sohbeti**. Messages are plain text, limited to 240 characters and one message per second per player; the room retains its latest 50 messages until it closes. Incoming messages preserve the current draft and reading position; **Son mesaja git** jumps to the newest message.

The host can open **Oyuncuları yönet** in the lobby, match or result screen and confirm **Çıkar** for another human participant, including a disconnected reservation. Removal revokes that session and its resume token, clears the participant from the match, and returns the connected participant to the landing screen with a notice. It is a session removal, not a permanent identity ban: someone with the invite can join again as a new participant. Bots continue to use their existing lobby removal controls.
